...
constexpr size_t HEIGHT = 9, WIDTH = 10;
...
// clang-format on
void write_to_buffer(decltype(zero) digit,
                     size_t position,
                     char buffer[BUFFER_HEIGHT][BUFFER_WIDTH])
{
    for (size_t i = 0; i < HEIGHT; ++i)
        for (size_t j = 0; j < WIDTH; ++j)
            buffer[i + 1][j + 1 + WIDTH * position] = digit[i][j];
}
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < 3; ++i) {
        write_to_buffer(numbers[number[i]], i, buffer);
    }

    for (size_t i = 0; i < BUFFER_HEIGHT; ++i) {
        for (size_t j = 0; j < BUFFER_WIDTH; ++j) std::cout << buffer[i][j];
